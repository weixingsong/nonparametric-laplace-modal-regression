...
# Testing hypothesis: 
# Y~Gamma given link function and linear part are correctely specified.（Model B.3）
# Test methods: MCCM2 
##Choose n=100,200,300

modelB3M2=function(n=100,B=100,su2=1,total=100,M=100)
 {
  set.seed(444)
   ...
   while(jj<total)
    {
      ...
      loc=g(thetax)
      r=0.577
      scale=(1/r)*(1-g(thetax))/(2+phi)        
      ystar<-rgumbel(n, loc=loc, scale=scale)  
      ymin=min(ystar)
      y=(ystar-ymin+0.00001) 
...
#btime=Sys.time()
#modelB3M2(100,100,1,total=100,M=100)
#Sys.time()-btime
