...
figureB14=function(n=100,B=100,su2=1)
 {
  set.seed(111)
...
#####################################
  ...
  # Generating hatX1
     ...
     hatsigmau=matrix(hatsigmau1,nrow=2)/nj   
   ...
   for(j in seq(100))
    {
      fp1[j]=mean(Pvalue<=pv[j])
      fp2[j]=mean(Pvalue<pv[j])
    }


   cat("\n")
   cat("Simulation Results in Figure B.14","\n")
   cat("n =",n,"","B =",B,"","su2 =",su2,"\n\n")
   cat("Empirical Level of MCCM1","\n\n")
...
btime=Sys.time()
postscript("results/figureB14.eps", width = 16, height = 6, 
