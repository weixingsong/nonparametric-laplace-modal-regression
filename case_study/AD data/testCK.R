...
testCK=function(B=50,link_type=1, covu_type=1) 
 { 
  path=getwd()
  source(paste0(path,"\\case_study\\AD data\\gammaTest.r"))
  source(paste0(path,"\\case_study\\AD data\\resampling.r"))
  ...
  for(j in seq(n))
   ...
   bet=optim(par=c(-0.71,-0.15,-0.08,2.93),fn=mccl,mydata=mydata)$par
   ...
   mytest1=test.CM(R, boot = 100, alpha = 0.05)$pvalue
   mytest2=test.KS(R, boot = 100, alpha = 0.05)$pvalue
