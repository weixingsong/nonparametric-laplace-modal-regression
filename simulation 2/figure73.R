...
# Test methods: MCCM1  
#Choose n=100,200,300 and beta_4=2,1.4,1.3,1.2, where beta_4 refers to the term in the bet2=phi/(g(thetax)+beta_4) in the following code.


figure73=function(n=100,B=100,su2=1,bt4=2)
 {
  set.seed(4444)
...
btime=Sys.time()
nsize <- c(100, 200, 300)
beta4_values <- c(2,1.4,1.3,1.2)

model23 <- matrix(NA, nrow = length(nsize), ncol = length(beta4_values))
rownames(model23) <- paste0("n=", nsize)
colnames(model23) <- paste0("beta4=", beta4_values)

for(i in seq_along(nsize)) {
  for(j in seq_along(beta4_values)) {
    model23[i, j] <- figure73(n = nsize[i], B = 100, su2 = 1, bt4 = beta4_values[j]) 
  }
}

postscript("results/figure73.eps", width = 8, height = 6, 
...
cat("Power of MCCM1 in Model 2.3","\n\n") 
