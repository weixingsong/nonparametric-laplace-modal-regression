...
# Testing hypothesis: 
# Y~Gamma given linear part and distribution are correctely specified.
# Test methods: MCCM1 
#Choose n=100,200,300 and beta_4=1,0.27,0.25,0.2, where beta_4 refers to the coefficient of x^2 in the function "gmis" in the following code.

figure72=function(n=100,B=100,su2=1,bt4=1)
 {
  set.seed(3333)
  ...
  while(jj<total)
   {
     ...
     gmis=function(x){g(x)/(1+bt4*x^2)}
     alph=1+phi               
     bt=phi/gmis(bt0+X1%*%bt1+X2*bt2)
...
btime=Sys.time()
nsize <- c(100, 200, 300)
beta4_values <- c(1,0.27,0.25,0.2)

model22 <- matrix(NA, nrow = length(nsize), ncol = length(beta4_values))
rownames(model22) <- paste0("n=", nsize)
colnames(model22) <- paste0("beta4=", beta4_values)

for(i in seq_along(nsize)) {
  for(j in seq_along(beta4_values)) {
    model22[i, j] <- figure72(n = nsize[i], B = 100, su2 = 1, bt4 = beta4_values[j])
  }
}

postscript("results/figure72.eps", width = 8, height = 6, 
...
cat("Simulation Results in Figure 7","\n")
cat("Power of MCCM1 in Model 2.2","\n\n") 
