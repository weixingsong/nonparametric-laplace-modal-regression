...
# Testing hypothesis: 
# Y~Gamma given link function and distrubution are correctely specified.
# Test methods: MCCM1    
##Choose n=100,200,300 and beta_4=-0.25,-0.08,-0.05, where beta_4 refer to bt3 in the following code.

figure71=function(n=100,B=100,su2=1,bt4=-0.25)
 {
  set.seed(2222)
  ...
  while(jj<total)
   {
     ...
     alph=1+phi               
     bt=phi/g(bt0+X1%*%bt1+X2*bt2+X1[,1]*X1[,1]*bt4)
...
btime=Sys.time()
nsize <- c(100, 200, 300)
beta4_values <- c(-0.25, -0.08, -0.05)

model21 <- matrix(NA, nrow = length(nsize), ncol = length(beta4_values))
rownames(model21) <- paste0("n=", nsize)
colnames(model21) <- paste0("beta4=", beta4_values)

for(i in seq_along(nsize)) {
  for(j in seq_along(beta4_values)) {
    model21[i, j] <- figure71(n = nsize[i], B = 100, su2 = 1, bt4 = beta4_values[j]) 
  }
}

postscript("results/figure71.eps", width = 8, height = 6, 
...
cat("Power of MCCM1 in Model 2.1\n\n")
