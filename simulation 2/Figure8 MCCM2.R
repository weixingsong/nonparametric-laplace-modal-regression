...
# Test methods: MCCM2    
# Choose n=100,200,300 and beta_4=1.3, where beta_4 refers to the term in the bet2=phi/(g(thetax)+beta_4) in the following code.


figure8M2=function(n=100,B=100,su2=1,bt4=1.3,total=100,M=100)
 {
  set.seed(4444)
  ...
  while(jj<total)
   {
     ...
     bet1=phi/g(thetax)
     alph2=alph1  
     bet2=phi/(g(thetax)+bt4)
     e.ind<-runif(n)
     y<-(e.ind<0.3)*rgamma(n,shape=alph1,rate=bet1)+(e.ind>0.3)*rgamma(n,shape=alph2,rate=bet2)
     ...
     if(betts$convergence==0)
      ...
      # Generating hatX1 using Tweedie's formula
      cc=1
      h=cc*n^(-1/5)
      difZmean1=kronecker(Zmean[,1],Zmean[,1],"-")
      difZmean2=kronecker(Zmean[,2],Zmean[,2],"-")
  
      fenz1=apply(matrix(difZmean1*dnorm(difZmean1/h)*dnorm(difZmean2/h),nrow=n),1,sum)/h^2
      fenz2=apply(matrix(difZmean2*dnorm(difZmean1/h)*dnorm(difZmean2/h),nrow=n),1,sum)/h^2
  
      fenm=apply(matrix(dnorm(difZmean1/h)*dnorm(difZmean2/h),nrow=n),1,sum) 
      ...
      fen=cbind(fenz1/fenm,fenz2/fenm)
  
      hatX1=Zmean+(fen/fenm)%*%hatsigmau
   ...
   Pvalue=mean(Pvalue<=0.05)
   return(Pvalue)
 }

#btime=Sys.time()
#figure8M2(100,100,1,1.3,total=100,M=100)
#Sys.time()-btime
