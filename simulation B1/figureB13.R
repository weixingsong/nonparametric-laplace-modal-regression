...
figureB13=function(n=2000,B=100,su2=1,dependence="TRUE")
 {
  set.seed(1234)
  ...
  for(kk in seq(total))
   {
     ...
     if(dependence=="TRUE")
      ...
      {
        X1=mvrnorm(n,mx1,covx1) # generate n sample from (X1,X2)
      }  
     bt0=-0.25
   ...
   cat("\n")
   cat("Simulation Results in Figure B.13","\n")
   cat("n =",n,"","B =",B,"","su2 =",su2,"\n\n")
   if(dependence=="TRUE")
    {
     cat("X1 and X2 are dependent","\n\n")
    } else
    {
     cat("X1 and X2 are independent","\n\n")
    }  

   library(latex2exp)
   resultNM=cbind(Mresult[,1],Nresult[,1],Mresult[,2],Nresult[,2],Mresult[,3],Nresult[,3],Mresult[,4],Nresult[,4])
   if(dependence=="TRUE")
    {
...
btime=Sys.time()
postscript("results/figureB13.eps", width = 12, height = 5,  
           horizontal = FALSE, paper = "special")

par(mfrow=c(1,2))
figureB13(2000,100,1,dependence="TRUE")
figureB13(2000,100,1,dependence="FALSE")
